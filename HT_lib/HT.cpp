...
int getNextBlock(int blockFile, int currentBlock){
    ...
    void* lastBlockBytes = static_cast<char*>(currentBlockPtr)+504;  //point to the last 8 bytes (size of int) of the block
    if (lastBlockBytes == NULL) return -1;
    memcpy(lastBlockBytes, &nextBlockIndex, sizeof(int));  //copy the index of the next block to the 
...
int HT_CreateIndex(char* filename, char attrType, char* attrName, int attrLength, int buckets){
    ...
    string HTIndexFilename(filename);
    ...
    //-------PROBLEM: HTIndexFilename NOT SAVED
    HT_info info = {blockFile, attrType, attrName, attrLength, buckets, HTIndexFilename};  
    void* HTinfoBlock;
...
int HT_InsertEntry(HT_info header_info, Record record){
    ...
    cout << header_info.indexFilename << endl;
    ...
    if (blockNumber == 0){  //if the current block is the HT_info block, go to the next block
        blockNumber = getNextBlock(header_info.fileDesc, blockNumber);
    }
